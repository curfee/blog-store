...
<!--more-->
# 初见面
原本因为有软件需要path ffmpeg才能正常导出文件,便去官网下载该文件<font size="1"><del>(一开始还错下源代码来着)</del></font>
然后有尝试着点击exe文件,见没反应之后很快就失去了兴趣,便很久没有管它了
不过就在今天,无意间刷到说是有款万能软件可以转换任何视频格式,竟发现up在用ffmpeg敲代码!!!
原来[ffmpeg](https://ffmpeg.org/)是转换软件啊...
## 初接触
看完后便兴冲冲地尝试了一下
```powershell
...
然后就是选项了,目前选项分为输入选项(放输入文件前),输出选项(放输出文件前),和全局选项(放哪里都是全局生效)

<font size="2">~~由于文档讲的太多了~~</font>列几个认为较为重要的选项
...
`-r`这个就是设置全局视频帧数`-r:v 10`就会得到每秒只有10帧的视频
...
<font size="1"><del>事实上效果比千禧年还烂</del></font>
...
`-vf`也是个强大的命令，可以同时设置视频长度,大小,滤镜,帧率,旋转等等效果。这里举几个常用的例子
...
```
~~不得不说批处理对空格异常敏感~~
这行里ffmpeg只有一行行是本体,同时也是最重要的一行,但由于这里没设置编码器质量和其他选项会导致其输出的视频质量出奇的低,~~我建议还是自己改改再用~~
***
...
最后就放一个纯音视频结合的例子吧
```powershell
ffmpeg -i "C:\Users\removable disk\Desktop\file_combie\spotifydown.com - The Last Time I Saw You - Housecat.mp3" -stream_loop -1 -i "C:\Users\removable disk\Desktop\file_combie\spr_1.webm" -b:v 40000 -c:v copy -c:a aac -b:a 256k -shortest "C:\Users\removable disk\Desktop\file_combie\spring.mp4"
```
